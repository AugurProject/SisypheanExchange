...
library TokenId {
	...
	function getTokenId(uint192 _universeId, uint56 _marketId, uint8 _outcome) internal pure returns (uint256 _tokenId) {
	...
	function getTokenIds(uint192 _universeId, uint56 _market, uint8[] memory _outcomes) internal pure returns (uint256[] memory _tokenIds) {
	...
	function unpackTokenId(uint256 _tokenId) internal pure returns (uint192 _universe, uint56 _market, uint8 _outcome) {
		assembly {
