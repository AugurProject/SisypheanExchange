...
library TokenId {
	...
	function getTokenId(uint256 _marketId, uint256 _outcome) internal pure returns (uint256 _tokenId) {
		bytes memory _tokenIdBytes = abi.encodePacked(uint248(_marketId), uint8(_outcome));
		assembly {
	...
	function unpackTokenId(uint256 _tokenId) internal pure returns (uint256 _market, uint256 _outcome) {
		assembly {
			_market := shr(8,  and(_tokenId, 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF00))
